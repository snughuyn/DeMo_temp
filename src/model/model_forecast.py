...
class ModelForecast(nn.Module):
    ...
    def forward(self, data):
        ...
        # 3가지 정보를 결합: Time Embed (B, 60, D) + Lane Form Embed (B, 60, D) + Offset Dist (B, 60, 1)
        lane_form_expand = lane_form_embed_agent.unsqueeze(1).expand(-1, mode.size(1), -1)
        lane_condition_input = torch.cat([mode, lane_form_expand, lane_offset_dist], dim=-1) # 🚨 offset 추가 🚨
