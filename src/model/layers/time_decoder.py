...
class TimeDecoder(nn.Module):
    ...
    def forward(self, mode, encoding, mask=None, **kwargs):
        ...
        if x_attr is not None and x_pos is not None:
            ...
            dfp_emb_is_available = False
            if "dfp_future_emb" in locals() and dfp_future_emb is not None:
                is_dfp_zero = (dfp_future_emb.abs().sum().item() < 1e-6)
             
                if not is_dfp_zero:
                    dfp_emb_is_available = True
    
            if dfp_emb_is_available:
